...
This script implements a comprehensive music recommendation system that leverages the Qwen2.5-Omni
...
def rank_songs_reranking(
    ...
    if mode in ("audio_query", "audio_only"):
        ...
        # Verify we have audio clips for audio modes
        if mode in ("audio_query", "audio_only") and len(clips) == 0:
            print("⚠️  No audio clips loaded - this may affect audio processing quality")
        
        # Generate white noise instead of loading real audio (commented out)
    ...
    sys_msg = {"role": "system", "content": [{"type": "text", "text": "You are a helpful music recommender."}]}
    ...
    # Process multimedia info for Qwen2.5-Omni
    conversations = [prefix_msgs]
    ...
    # Process inputs with multimedia support
    try:
        ...
        print("✅ Prefix inputs processed successfully")
    except Exception as e:
        print(f"❌ Error processing prefix inputs: {e}")
        return []
    ...
    has_audio = any(conversation_audios) if conversation_audios else False
    ...
    with torch.no_grad():
        ...
        for cand in candidates:  # Still score all original candidates
            ...
            # Process full conversation with multimedia
            full_conversations = [full_msgs]
            full_audios, full_images, full_videos = process_mm_info(full_conversations, use_audio_in_video=False)
            
            full_text = processor.apply_chat_template(full_conversations, add_generation_prompt=False, tokenize=False)
            inp = processor(
                text=full_text,
                audio=full_audios,
                images=None,
            ...
            # Forward pass with Qwen2.5-Omni (automatically handles multimodal inputs)
            out = model(
                **inp,
                return_dict=True,
                use_audio_in_video=False
    ...
    with torch.no_grad():
        out_ids = model.generate(
            **model_inputs,
            ...
            do_sample=False,
            use_audio_in_video=False,
            return_audio=False  # Only return text, not speech
        )
