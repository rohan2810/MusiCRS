...
class SALMONN(nn.Module):
    def __init__(self, ckpt, whisper_path, beats_path, vicuna_path,
                 ...
                 device_map="auto", audio_device: Optional[Union[str, torch.device]] = None):

        super().__init__()

        # Resolve audio side device (keep LLaMA on device_map)
        if audio_device is None:
            audio_device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        else:
            audio_device = torch.device(audio_device)
        ...
        self.llama_model = LlamaForCausalLM.from_pretrained(
            vicuna_path, 
            torch_dtype=compute_dtype,  # Keep original paper's float16 setting
        ...
        # Move audio pathway modules to chosen device/dtype for fast inference
        # Do NOT move self.llama_model (managed by device_map)
        self.speech_encoder.to(audio_device).to(compute_dtype)
        self.beats.to(audio_device).to(compute_dtype)
        self.speech_Qformer.to(audio_device).to(compute_dtype)
        self.speech_query_tokens.data = self.speech_query_tokens.data.to(device=audio_device, dtype=compute_dtype)
        self.ln_speech.to(audio_device).to(compute_dtype)
        self.ln_audio.to(audio_device).to(compute_dtype)
        self.speech_llama_proj.to(audio_device).to(compute_dtype)
    ...
    def forward(
        ...
        for speech_embed, audio_embed, input_id, label in zip(speeches, audios, input_ids, labels):
            # Use the device of the speech encoder for consistency
            model_device = next(self.speech_encoder.parameters()).device
            model_dtype = next(self.speech_encoder.parameters()).dtype
            speech_embed, audio_embed = speech_embed.to(model_device, dtype=model_dtype), audio_embed.to(model_device, dtype=model_dtype)
            # auditory embeds
    ...
    def generate(
        ...
        # Determine devices/dtypes:
        # - audio/whisper path runs on the speech encoder's device (often CPU or a single GPU)
        # - text/LLM path must run on the embed_tokens' device when using device_map (multi-GPU)
        model_device = next(self.speech_encoder.parameters()).device
        model_dtype = next(self.speech_encoder.parameters()).dtype
        embed_tokens = self.llama_model.model.model.embed_tokens if self.lora else self.llama_model.model.embed_tokens
        text_device = embed_tokens.weight.device
        text_dtype = embed_tokens.weight.dtype
        ...
        spectrogram = self.feature_extractor(wav, return_tensors="pt", sampling_rate=16000).input_features.to(
            model_device, dtype=model_dtype)  # [1, 80, 3000]
        speech_embeds = self.speech_encoder(spectrogram, return_dict=True).last_hidden_state

        # beats
        raw_wav = torch.from_numpy(wav).to(model_device).unsqueeze(0)
        audio_padding_mask = torch.zeros(raw_wav.shape, device=model_device).bool()
        ...
        # Move speech embeddings to the LLM's input device/dtype for model-parallel inference
        speech_embeds = speech_embeds.to(device=text_device, dtype=text_dtype)

        # Tokenize and embed prompts on the LLM embed device
        prompt_left_ids = self.tokenizer(prompt_left, return_tensors="pt", add_special_tokens=False).to(
            text_device).input_ids
        ...
        bos_idx = torch.ones([1, 1], dtype=torch.long, device=text_device) * self.tokenizer.bos_token_id
        bos_embeds = embed_tokens(bos_idx)

        embeds = torch.cat([bos_embeds, prompt_left_embeds, speech_embeds, prompt_right_embeds], dim=1)
        atts = torch.ones(embeds.size()[:-1], dtype=torch.long, device=text_device)
